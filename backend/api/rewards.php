...
$rewards = $pdo->query("SELECT id, title, description, image_url, reward_code, product_url, points_cost, stock, is_active, sort_order FROM {$rewardTable} WHERE is_active = 1 ORDER BY sort_order ASC, id DESC")->fetchAll(PDO::FETCH_ASSOC);
$user = null;
$redemptions = [];
...
if ($uid) {
    ...
    $redSt = $pdo->prepare(
        "SELECT rr.*, r.title AS reward_title, r.image_url, r.product_url
